...
class PlateRecognition:
    def __init__(self,):
        ...
        detect_model_path = r'weights\best_yolov5s_final.pt'
        rec_model_path = r"weights/plate_rec_color.pth"
        self.detect_model = init_model_detect(detect_model_path,device)
        self.plate_rec_model=init_model(device,rec_model_path,is_color=True)
        self.img_size = 416
        
    def get_plate_result(self,img):
         outputs=get_plate_result_all(img,self.detect_model,self.plate_rec_model,(self.img_size,self.img_size))
         return outputs
...
if __name__ == "__main__":
    ...
    for pic_ in file_list:
        ...
        img = cv2.imread(pic_)
        im0 = copy.deepcopy(img)
        outputs=plate_rec.get_plate_result(img)
